#!/usr/bin/env bash
# Bluetooth manager with wofi (HID-aware + actions menu with back + robust MAC selection)
...
WOFI_DEVICES='wofi --dmenu --prompt "Bluetooth" --allow-markup'
WOFI_GENERIC='wofi --dmenu --allow-markup'

LOGFILE="$HOME/.local/share/bluetooth-wofi.log"
...
device_is_hid()   { bluetoothctl info "$1" | grep -q "Human Interface Device"; }
...
toggle_power(){
	if power_on; then
		...
		rfkill list bluetooth | grep -q 'Blocked: yes' && rfkill unblock bluetooth && sleep 2
...
toggle_connection(){
	...
	if device_connected "$mac"; then
		...
		sleep 2
		if device_connected "$mac"; then notify "Disconnect failed"; else notify "Disconnected"; fi
	else
		log "Connecting $mac"; notify "Connecting..."
		bluetoothctl connect "$mac" >>"$LOGFILE" 2>&1
		for _ in 1 2 3; do sleep 2; device_connected "$mac" && { notify "Connected"; return; }; done
...
actions_menu(){
    ...
    items="$pwr\n$scn\n$prb\n$dsc\n$forget\n$back"
    choice="$(echo -e "$items" | $WOFI_GENERIC)" || return
    case "$choice" in
        *"Power"*)        toggle_power ;;
        *"Scan"*)         toggle_scan ;;
        ...
        *"Forget All"*)
            if echo -e "No\nYes" | $WOFI_GENERIC --prompt "Forget all devices?" | grep -q Yes; then
                bluetoothctl devices | awk '!seen[$2]++ {print $2}' | while read -r mac; do bluetoothctl remove "$mac"; done
...
device_menu(){
    local device="$1"
    local name="$(echo "$device" | cut -d ' ' -f 3-)"
    ...
    local items="$connected\n$paired\n$trusted"
    choice="$(echo -e "$items" | $WOFI_GENERIC --prompt "$name")" || { run_scan_and_show; return; }
    case "$choice" in
...
show_devices(){
    ...
    choice="$(echo "$rows" | $WOFI_DEVICES)" || return
...
case "$1" in
	--status) 
		if power_on; then
			mapfile -t devs < <(bluetoothctl paired-devices | awk '{print $2}')
			for d in "${devs[@]}"; do
				if device_connected "$d"; then echo "$GREEN$ICON_BT$END"; exit; fi
				if device_is_hid "$d" && device_paired "$d"; then echo "$YELLOW$ICON_BT$END"; exit; fi
			done
